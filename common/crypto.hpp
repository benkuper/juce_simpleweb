...
namespace SimpleWeb {
	...
	public:
		...
		static std::string md5(const std::string& input, std::size_t iterations = 1) noexcept {
			...
			juce::MemoryBlock b;
			juce::MD5 md5(reinterpret_cast<const unsigned char*>(&input[0]), (size_t)input.size());
			b = md5.getRawChecksumData();// reinterpret_cast<unsigned char*>(&hash[0]));
		...
		static std::string sha256(const std::string& input, std::size_t iterations = 1) noexcept {
			...
			juce::SHA256 sha(reinterpret_cast<const unsigned char*>(&input[0]), input.size());
			juce::MemoryBlock b = sha.getRawData();//  reinterpret_cast<unsigned char*>(&hash[0]));

			for (std::size_t c = 1; c < iterations; ++c)
				b = juce::SHA256(b.getData(), b.getSize()).getRawData();
